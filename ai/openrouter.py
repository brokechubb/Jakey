...
class OpenRouterAPI:
    ...
    def __init__(self):
        ...
        self.enabled = bool(
            OPENROUTER_ENABLED and self.api_key
        )  # Ensure boolean, not API key leak
        ...
        logger.info(
            f"OpenRouter API initialized: enabled={self.enabled}, model={self.default_model}, timeout={self.text_timeout}s, rate_limit={self.rate_limit}/min"
        )
    ...
    def _reset_daily_counter_if_needed(self) -> None:
        ...
        if self._last_reset_date != current_date:
            logger.info(
                f"OpenRouter: New UTC day ({current_date}), resetting free request counter"
            )
            self._free_requests_today = 0
    ...
    def get_api_limits(self, force_refresh: bool = False) -> Optional[OpenRouterLimits]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                daily_limit = (
                    self.FREE_MODEL_DAILY_LIMIT_FREE_TIER
                    if is_free_tier
                ...
                logger.warning(
                    f"OpenRouter: Failed to get limits (HTTP {response.status_code})"
                )
                return None
    ...
    def check_rate_limits(self) -> Dict[str, Any]:
        ...
        with self._rate_lock:
            ...
            if len(self._requests) >= self.rate_limit:
                ...
                result["reason"] = (
                    f"Per-minute rate limit exceeded ({self.rate_limit}/min)"
                )
                return result
    ...
    def generate_text(
        ...
        if self._is_rate_limited(current_time):
            return {
                "error": "Per-minute rate limit exceeded. Please try again later.",
                "rate_limited": True,
            }
        ...
        if tools and model not in FUNCTION_CALLING_MODELS:
            ...
            logger.debug(
                f"Model {original_model} doesn't support function calling. Switching to {model} for tool use."
            )
        ...
        if use_fallback_routing:
            ...
            logger.debug(
                f"OpenRouter: Using fallback routing with models: {models_to_use}"
            )
        ...
        if tools and len(tools) > 20:  # Large tool arrays need more time
            current_timeout = min(
                self.text_timeout * 2, 120
            )  # Double timeout, max 2 minutes
            logger.debug(
                f"OpenRouter: Increased timeout to {current_timeout}s for {len(tools)} tools"
            )

        for attempt in range(max_retries + 1):
            try:
                logger.debug(
                    f"OpenRouter: Making request to model {model} (Attempt {attempt + 1}/{max_retries + 1})"
                )
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(result, dict) and "error" in result:
                        ...
                        raw_error = (
                            error_metadata.get("raw", "")
                            if isinstance(error_metadata, dict)
                            else ""
                        )
                        retriable_codes = [500, 502, 503, 504]

                        # Also retry on 404 "No matching route found" - provider routing failures
                        is_route_failure = (
                            error_code == 404 and "No matching route found" in raw_error
                        )

                        if (
                            error_code and error_code in retriable_codes
                        ) or is_route_failure:
                            ...
                            if attempt < max_retries:
                                ...
                                logger.error(
                                    f"OpenRouter: Upstream error {error_code} - Max retries reached"
                                )
                    ...
                    error_msg = (
                        "OpenRouter: Negative credit balance. Add credits to continue."
                    )
                    logger.error(f"OpenRouter: {error_msg}")
                    ...
                    logger.error(
                        f"OpenRouter request model: {model}, tools provided: {bool(tools)}, tools count: {len(tools) if tools else 0}"
                    )
                    ...
                    if attempt < max_retries:
                        sleep_time = retry_delay * (2**attempt) + random.uniform(0, 1)
                        logger.warning(
                            f"OpenRouter: {error_msg} (provider error). Retrying in {sleep_time:.2f}s..."
                        )
                        time.sleep(sleep_time)
                        continue
                    else:
                        logger.error(
                            f"OpenRouter: {error_msg} - Max retries reached - {response.text}"
                        )
                        return {"error": error_msg}
                else:
                    error_msg = f"OpenRouter HTTP {response.status_code}"
                    ...
                    # Special handling for 404 provider routing errors - these are retriable
                    if response.status_code == 404:
                        # Check for "No matching route found" - provider is temporarily unavailable
                        if "No matching route found" in response.text:
                            if attempt < max_retries:
                                sleep_time = retry_delay * (
                                    2**attempt
                                ) + random.uniform(0, 1)
                                logger.warning(
                                    f"OpenRouter: Provider routing failed (404). Retrying in {sleep_time:.2f}s... (Attempt {attempt + 2}/{max_retries + 1})"
                                )
                                time.sleep(sleep_time)
                                continue
                            else:
                                logger.error(
                                    f"OpenRouter: Provider routing failed - Max retries reached"
                                )
                                return {"error": error_msg}

                        # Check for "All providers have been ignored" error
                        if "All providers have been ignored" in response.text:
                            logger.warning(
                                "OpenRouter: All providers ignored - retrying without provider ignore list"
                            )
                            ...
                            if "provider" in payload:
                                ...
                                try:
                                    logger.debug(
                                        f"OpenRouter: Retrying request to model {model} without provider ignore"
                                    )
                                    response = requests.post(
                                        self.api_url,
                                        headers=self._get_headers(),
                                    ...
                                    if response.status_code == 200:
                                        ...
                                        if is_free:
                                            self._record_free_request()
                                        logger.debug(
                                            f"OpenRouter: Successful response from {model} after removing provider ignore"
                                        )
                                        return result
                                except Exception as retry_error:
                                    logger.error(
                                        f"OpenRouter: Retry without provider ignore failed: {retry_error}"
                                    )
                ...
                if "Response ended prematurely" in str(e) or "Connection broken" in str(
                    e
                ):
                    if attempt < max_retries:
                        sleep_time = retry_delay * (2**attempt) + random.uniform(0, 1)
                        logger.warning(
                            f"OpenRouter: Connection issue. Retrying in {sleep_time:.2f}s... (Attempt {attempt + 2}/{max_retries + 1})"
                        )
                        time.sleep(sleep_time)
                        continue
    ...
    def get_generation_stats(self, generation_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json().get("data", {})
                logger.debug(
                    f"OpenRouter: Generation stats for {generation_id}: {data}"
                )
                return data
            else:
                logger.warning(
                    f"OpenRouter: Failed to get generation stats (HTTP {response.status_code})"
                )
                return None
